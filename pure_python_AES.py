nb = 4  # number of columns of State (in AES it is always 4)
nr = 10  # number of rounds
nk = 4  # altered key length

# dict used in byte substitution
# hexadecimal to decimal values
hex_symbols_to_int = {'a': 10, 'b': 11, 'c': 12, 'd': 13, 'e': 14, 'f': 15}

# substitution box
sbox = [
    
]

# inverse substitution box
inv_sbox = [
    
]

# round constants
rcon = [[0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36],
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
]


def encrypt(input_bytes, key):
    '''
    encrypts plaintext

    :param input_bytes: plaintext
    :param key: 16 character long key
    :return: output
    '''

    # data entry preparation
    state = [[] for j in range(4)]
    for r in range(4):
        for c in range(nb):
            state[r].append(input_bytes[r + 4 * c])

    key_schedule = key_expansion(key)

    state = add_round_key(state, key_schedule)

    for rnd in range(1, nr):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, key_schedule, rnd)

    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, key_schedule, rnd + 1)

    output = [None for i in range(4 * nb)]
    for r in range(4):
        for c in range(nb):
            output[r + 4 * c] = state[r][c]

    return output


def decrypt(cipher, key):
    '''
    decrypts encrypted text

    :param cipher: encrypted text
    :param key: 16 character long key
    :return: output
    '''

    # data entry preparation
    state = [[] for i in range(nb)]
    for r in range(4):
        for c in range(nb):
            state[r].append(cipher[r + 4 * c])

    key_schedule = key_expansion(key)

    state = add_round_key(state, key_schedule, nr)

    rnd = nr - 1
    while rnd >= 1:
        state = shift_rows(state, inv=True)
        state = sub_bytes(state, inv=True)
        state = add_round_key(state, key_schedule, rnd)
        state = mix_columns(state, inv=True)

        rnd -= 1

    state = shift_rows(state, inv=True)
    state = sub_bytes(state, inv=True)
    state = add_round_key(state, key_schedule, rnd)

    output = [None for i in range(4 * nb)]
    for r in range(4):
        for c in range(nb):
            output[r + 4 * c] = state[r][c]

    return output


def sub_bytes(state, inv=False):
    '''
    substitutes bytes from sbox or inv_sbox
    replaces the element of State with an indexed hex value

    :param state: element of State
    :param inv: if false then it is encrypting and vice versa
    :return: state
    '''

    if inv == False:  # encrypt
        box = sbox
    else:  # decrypt
        box = inv_sbox

    for i in range(len(state)):
        for j in range(len(state[i])):
            row = state[i][j] // 0x10
            col = state[i][j] % 0x10

            # Our Sbox is a flat array, not a bable. So, we use this trich to find elem:
            # And DO NOT change list sbox! if you want it to work
            box_elem = box[16 * row + col]
            state[i][j] = box_elem

    return state


def shift_rows(state, inv=False):
    '''
    shifts the elements of State based on rows

    :param state: element of State
    :param inv: if false then it is encrypting and vice versa
    :return: state
    '''

    count = 1

    if inv == False:  # encrypting
        for i in range(1, nb):
            state[i] = left_shift(state[i], count)
            count += 1
    else:  # decryptionting
        for i in range(1, nb):
            state[i] = right_shift(state[i], count)
            count += 1

    return state


def mix_columns(state, inv=False):
    '''
    multiplies every column of State with a fixed polynomial in a Galois Field
    X(x) = {03}x**3+{01}x**2+{01}x+{02}

    :param state: element of State
    :param inv: if false then it is encrypting and vice versa
    :return: state
    '''

    for i in range(nb):

        if inv == False:  # encryption
            s0 = mul_by_02(state[0][i]) ^ mul_by_03(state[1][i]) ^ state[2][i] ^ state[3][i]
            s1 = state[0][i] ^ mul_by_02(state[1][i]) ^ mul_by_03(state[2][i]) ^ state[3][i]
            s2 = state[0][i] ^ state[1][i] ^ mul_by_02(state[2][i]) ^ mul_by_03(state[3][i])
            s3 = mul_by_03(state[0][i]) ^ state[1][i] ^ state[2][i] ^ mul_by_02(state[3][i])
        else:  # decryption
            s0 = mul_by_0e(state[0][i]) ^ mul_by_0b(state[1][i]) ^ mul_by_0d(state[2][i]) ^ mul_by_09(state[3][i])
            s1 = mul_by_09(state[0][i]) ^ mul_by_0e(state[1][i]) ^ mul_by_0b(state[2][i]) ^ mul_by_0d(state[3][i])
            s2 = mul_by_0d(state[0][i]) ^ mul_by_09(state[1][i]) ^ mul_by_0e(state[2][i]) ^ mul_by_0b(state[3][i])
            s3 = mul_by_0b(state[0][i]) ^ mul_by_0d(state[1][i]) ^ mul_by_09(state[2][i]) ^ mul_by_0e(state[3][i])

        state[0][i] = s0
        state[1][i] = s1
        state[2][i] = s2
        state[3][i] = s3

    return state


def key_expansion(key):
    '''
    makea a list of round keys for the function add round key

    :param key: 16 character long key
    :return:
    '''

    key_symbols = [ord(symbol) for symbol in key]

    # the cipher key should contain 16 symbols to fill a 4 by 4 table
    # if it is less than complement the key with 0x01
    if len(key_symbols) < 4 * nk:
        for i in range(4 * nk - len(key_symbols)):
            key_symbols.append(0x01)

    # make the cipher key
    key_schedule = [[] for i in range(4)]
    for r in range(4):
        for c in range(nk):
            key_schedule[r].append(key_symbols[r + 4 * c])

    # continues to fill the key schedule
    for col in range(nk, nb * (nr + 1)):  # col - column number
        if col % nk == 0:
            # take shifted (col - 1)th column...
            tmp = [key_schedule[row][col - 1] for row in range(1, 4)]
            tmp.append(key_schedule[0][col - 1])

            # changes the elements using sbox
            for j in range(len(tmp)):
                sbox_row = tmp[j] // 0x10
                sbox_col = tmp[j] % 0x10
                sbox_elem = sbox[16 * sbox_row + sbox_col]
                tmp[j] = sbox_elem

            # XOR of 3 columns
            for row in range(4):
                s = (key_schedule[row][col - 4]) ^ (tmp[row]) ^ (rcon[row][int(col / nk - 1)])
                key_schedule[row].append(s)

        else:
            # XOR of 2 columns
            for row in range(4):
                s = key_schedule[row][col - 4] ^ key_schedule[row][col - 1]
                key_schedule[row].append(s)

    return key_schedule


def add_round_key(state, key_schedule, round=0):
    '''
    combines State and the key schedule together
    XOR of State and the round schedule as a part of key schedule

    :param state: element of State
    :param key_schedule: the key schedule
    :param round: the round number
    :return: state
    '''

    for col in range(nk):
        # nb*round is a shift which indicates start of a part of the KeySchedule
        s0 = state[0][col] ^ key_schedule[0][nb * round + col]
        s1 = state[1][col] ^ key_schedule[1][nb * round + col]
        s2 = state[2][col] ^ key_schedule[2][nb * round + col]
        s3 = state[3][col] ^ key_schedule[3][nb * round + col]

        state[0][col] = s0
        state[1][col] = s1
        state[2][col] = s2
        state[3][col] = s3

    return state



# rotates the array count times
def left_shift(array, count):

    res = array[:]
    for i in range(count):
        temp = res[1:]
        temp.append(res[0])
        res[:] = temp[:]

    return res

# rotates the array count times
def right_shift(array, count):

    res = array[:]
    for i in range(count):
        tmp = res[:-1]
        tmp.insert(0, res[-1])
        res[:] = tmp[:]

    return res

# multiply by 2 in a Galois Field
def mul_by_02(num):
    if num < 0x80:
        res = (num << 1)
    else:
        res = (num << 1) ^ 0x1b

    return res % 0x100

# multiply by 3 in a Galois Field
def mul_by_03(num):
    return (mul_by_02(num) ^ num)

# multiply by 9 in a Galois Field
def mul_by_09(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ num

# multiply by 11 in a Galois Field
def mul_by_0b(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(num) ^ num

# multiply by 13 in a Galois Field
def mul_by_0d(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(mul_by_02(num)) ^ num

# multiply by 14 in a Galois Field
def mul_by_0e(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(mul_by_02(num)) ^ mul_by_02(num)



# main program
if __name__ == '__main__':

    import os
    import time

    while True:
        print('enter 1 to encrypt and 2 to decrypt')
        way = input()
        if way not in ['1', '2']:
            print('ERROR: invalid entry')
            continue
        else:
            break

    while True:
        print('enter file name')
        input_path = os.path.abspath(input())

        if os.path.isfile(input_path):
            break
        else:
            print('ERROR: file not found')
            continue
    print()

    while True:
        print('enter key for encryption/decryption\nthe key must be less than 16 symbols')
        key = input()

        if len(key) > 16:
            print('ERROR: key too long')
            continue

        for symbol in key:
            if ord(symbol) > 0xff:
                print('ERROR: invalid character entry')
                continue
        break

    time_before = time.time()

    # input data
    with open(input_path, 'rb') as f:
        data = f.read()

    if way == '1':
        crypted_data = []
        temp = []
        for byte in data:
            temp.append(byte)
            if len(temp) == 16:
                crypted_part = encrypt(temp, key)
                crypted_data.extend(crypted_part)
                del temp[:]
        else:

            # more padding
            if 0 < len(temp) < 16:
                empty_spaces = 16 - len(temp)
                for i in range(empty_spaces - 1):
                    temp.append(0)
                temp.append(1)
                crypted_part = encrypt(temp, key)
                crypted_data.extend(crypted_part)

        out_path = os.path.join(os.path.dirname(input_path), 'AES128_encrypted' + os.path.basename(input_path))

        # output data
        with open(out_path, 'xb') as ff:
            ff.write(bytes(crypted_data))

    else:  # if way == '2'
        decrypted_data = []
        temp = []
        for byte in data:
            temp.append(byte)
            if len(temp) == 16:
                decrypted_part = decrypt(temp, key)
                decrypted_data.extend(decrypted_part)
                del temp[:]
        else:

            # more padding
            if 0 < len(temp) < 16:
                empty_spaces = 16 - len(temp)
                for i in range(empty_spaces - 1):
                    temp.append(0)
                temp.append(1)
                decrypted_part = encrypt(temp, key)
                decrypted_data.extend(crypted_part)

        out_path = os.path.join(os.path.dirname(input_path), 'decrypted_' + os.path.basename(input_path))

        # output data
        with open(out_path, 'xb') as ff:
            ff.write(bytes(decrypted_data))

    time_after = time.time()

print('new file path: ' + out_path + '\n'+ time_after - time_before + ' seconds')